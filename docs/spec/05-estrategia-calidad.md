...
| Tipo | Peso | Por qué |
|---|---|---|
| **Integración de API contra PostgreSQL real** | ~65 % | **155 pruebas** (137 integración + 18 unitarias: 12 de `pg-error` y 6 de `pool-config`). Una sola prueba de `POST /projects → POST /tasks → PATCH status → GET` ejercita middleware, Zod, ruta, repositorio, SQL parametrizado y las restricciones reales del motor. Ningún otro tipo de prueba cubre tanto por línea escrita. |
